...
/// <summary>
///     An agent can be thought of as a bit like an Actor, except that the state can be read at any time via the Value
///     property,
///     and functions are sent to the agent instead of messages. The Value property can always be read without blocking,
///     functions
///     sent to a specific agent are processed in the order they are received, and only one function is processed at a
///     time. Functions
...
public class Agent<T> : ARef<T>
{
    ...
    public static int ReleasePendingSends()
    {
        ...
        foreach (var send in sends) send.Enqueue();
    ...
    private sealed class Action : IAction
    {
        ...
        public Action(Agent<T> agent, UpdateFn fn, IExecutor executor)
        {
            _agent = agent;
            _fn = fn;
            _executor = executor;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (_agent.ErrorHandler != null)
                        ...
                        {
                            if (_agent.ErrorMode == AgentGlobals.ErrorModes.Continue) exception = null;
                        }
                }
                ...
                if (exception == null && !next._queue.IsEmpty) next._queue.Peek().Execute();
            }
            finally
            {
                AgentGlobals.ResetNested();
